...
class MediaCodecEncoder(
    ...
    private var mFirstPresentationTimeUs: Long = -1L  // 初始时间戳
    private var mLastPresentationTimeUs: Long = -1L   // 上一次时间戳
    private var mEncodedDurationUs: Long = 0L         // 累积时长
    ...
    private fun initEncoding() {
        ...
        try {
            mContainer = format.getContainer(config.outputPath)
        } catch (e: Exception) {
            onError(e)
        }
    }
    ...
    private fun processOutputBuffer(codec: MediaCodec, index: Int, info: MediaCodec.BufferInfo) {
        try {
            ...
            if (!isCodecData && info.size != 0) {
                ...
                // 第一次编码数据，初始化基准时间戳
                if (mFirstPresentationTimeUs < 0) {
                    mFirstPresentationTimeUs = info.presentationTimeUs
                    mLastPresentationTimeUs = info.presentationTimeUs
                    mEncodedDurationUs = 0
                } else {
                    // 只有当前时间戳比上一次大才更新
                    if (info.presentationTimeUs > mLastPresentationTimeUs) {
                        mEncodedDurationUs = info.presentationTimeUs - mFirstPresentationTimeUs
                        mLastPresentationTimeUs = info.presentationTimeUs
                    }
                }

                // 实时回调编码时长（毫秒）
                listener.onEncodeDuration(mEncodedDurationUs / 1000)
            }
